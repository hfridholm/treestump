...
U64 generate_occupancies_magic_number(U64 occupancies[4096], int relevantBits, U64 attacks[4096], U64 attackMask)
{
    ...
    for (int count = 0; count < 100000000; count++)
    {
        ...
        for(int index = 0; !fail && index < occupancyIndicies; index++)
        {
            int magicIndex = ((occupancies[index] * magicNumber) >> (BOARD_SQUARES - relevantBits));
...
U64 generate_square_magic_number(Square square, int relevantBits, bool bishop)
{
    ...
    {
        occupancies[index] = create_index_occupancy(index, attackMask, relevantBits);
        
        attacks[index] = bishop ? calculate_bishop_attacks(square, occupancies[index]) : calculate_rook_attacks(square, occupancies[index]);
    }
